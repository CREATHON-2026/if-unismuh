...
/*
 * Disalin dari registri beUI, lalu disesuaikan di tiga tempat. Kalau berkas ini
 * dipasang ulang lewat `shadcn add`, penyesuaiannya hilang dan build gagal lagi.
 *
 * 1. `useRef<HTMLElement | null>` — beUI ditulis untuk tipe React 19, tempat
 *    `current` bisa ditulis. Proyek ini React 18, tempat `current` readonly.
 *
 * 2. `.at(-1)` diganti indeks biasa. Itu ES2022, sedangkan proyek ini menyetel
 *    ES2020. Menaikkan `lib` di tsconfig akan lebih singkat, tapi ia menaikkan
 *    lantai browser SELURUH aplikasi demi satu komponen — dan pengguna kita
 *    memakai ponsel murah dengan WebView lama. Menurunkan lantai itu adalah
 *    kegagalan yang tidak akan terlihat di laptop siapa pun di tim ini.
 *
 * 3. `"use client"` dibiarkan. Ia arahan Next.js dan tidak berarti apa-apa di
 *    Vite — dihapus pun tidak apa-apa, tapi membiarkannya memudahkan
 *    membandingkan berkas ini dengan sumbernya saat beUI diperbarui.
 */
