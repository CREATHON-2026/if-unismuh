      ...
      {open ? (
        <PresenceGate key="panel-layer">
          {({ isPresent, gate }) => (
            ...
            <div
              // React 18 belum mengenal `inert` sebagai prop (baru di React 19),
              // padahal atributnya sudah lama didukung peramban. Dikirim sebagai
              // atribut HTML mentah lewat spread: string kosong menyalakannya,
              // objek kosong menghilangkannya sama sekali. Diubah dari bentuk
              // asli beUI — kalau komponen ini dipasang ulang dari registri,
              // penyesuaian ini perlu diterapkan lagi.
              {...(!isPresent ? ({ inert: '' } as Record<string, unknown>) : {})}
              className={cn(
