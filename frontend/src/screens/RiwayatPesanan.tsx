...
export function RiwayatPesanan() {
  ...
  return (
    <Layar kembali={() => nav('/pesanan')} atas>
      <h1 className="text-judul font-bold tracking-[-0.02em] text-tinta">Riwayat Pesanan</h1>

      {r && (
        <div className="mt-4">
          <KartuHero
            label="Untung dari pesanan yang sudah selesai"
            nilai={`${r.untung < 0 ? '\u2212' : '+'} ${formatRupiah(Math.abs(r.untung))}`}
            nada={r.untung < 0 ? 'rugi' : 'untung'}
            catatan={
              r.gagal > 0
